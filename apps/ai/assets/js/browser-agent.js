...
9. Scroll the page (for SPAs that load content dynamically):
<tool name="scroll"><direction>down</direction><amount>500</amount></tool>

10. Wait for a duration in milliseconds (for page loading):
<tool name="wait"><ms>2000</ms></tool>

11. Capture a screenshot of the current page (requires screenshot API configured in Settings > Browser):
<tool name="get_page_screenshot"></tool>

12. Finish the task and return a final answer:
...
async function executeBrowserTool({ name, args }, browserState, onProgress, signal) {
  switch (name) {
    case "navigate": {
      ...
      // Use remote browser if configured
      if (browserState.remoteBrowser) {
        try {
          await browserState.remoteBrowser.navigate(url);
          const content = await browserState.remoteBrowser.getPageContent();
          browserState.currentUrl = url;
          browserState.lastFetchedUrl = url;
          browserState.pageText = content.text || content.pageText || '';
          browserState.pageLinks = content.links || [];
          browserState.pageTitle = content.title || '';

          const viewUrl = browserState.remoteBrowser.getViewUrl();
          if (viewUrl && browserState.navigateIframe) {
            browserState.navigateIframe(viewUrl);
          }
          ...
          if (onProgress) {
            onProgress({
              ...
              textLength: browserState.pageText.length,
              pageText: browserState.pageText,
            });
          }

          const preview = (browserState.pageText || '').slice(0, 8000);
          return `Navigated to ${url} via remote browser\nTitle: ${browserState.pageTitle}\n\nPage content (first 8000 chars):\n${preview}`;
        } catch (e) {
          return `Error: Remote browser navigation failed. ${e.message || e}`;
        }
      }
      ...
      // Attempt srcdoc rendering for visual display (bypasses X-Frame-Options)
      if (browserState.renderFetchedHtml) {
        try {
          const resp = await fetch(url, {
            signal,
            headers: { Accept: "text/html,*/*" },
          ...
          if (resp.ok) {
            ...
            if (bodyMatch) {
              browserState.renderFetchedHtml(bodyMatch[1], url);
            } else {
              // If no body tag, render the full HTML
              browserState.renderFetchedHtml(fullHtml, url);
            }
          }
        } catch {
          // srcdoc rendering is best-effort; fall back to iframe
        }
      }
    ...
    case "scroll": {
      ...
      const amount = parseInt(args.amount || '500', 10);

      if (browserState.scrollIframe) {
        browserState.scrollIframe(direction, amount);
      }

      // Re-fetch page content after scroll (for SPAs that load content dynamically)
      if (browserState.currentUrl) {
        try {
          const pageData = await parsePageHtml(browserState.currentUrl, signal);
          browserState.pageText = pageData.pageText;
          browserState.pageLinks = pageData.links;
          browserState.pageTitle = pageData.title;
        } catch {
          // If re-fetch fails, keep existing content
        }
      }

      if (onProgress) {
        onProgress({ type: "scroll", direction, amount });
      }

      return `Scrolled ${direction} by ${amount}px. Page content re-fetched.`;
    }

    case "wait": {
      const ms = parseInt(args.ms || '2000', 10);
      ...
      if (onProgress) {
        onProgress({ type: "wait", ms });
      }

      return `Waited ${ms}ms. Page content re-fetched.`;
    }

    case "get_page_screenshot": {
      const screenshotApi = browserState.screenshotApiUrl;
      if (!screenshotApi) {
        return "Error: No screenshot API configured. Add a screenshot service URL in Settings > Browser.";
      }

      const targetUrl = browserState.currentUrl;
      if (!targetUrl) return "Error: No page loaded. Navigate to a URL first.";
      ...
      try {
        const resp = await fetch(screenshotApi, {
          ...
          body: JSON.stringify({ url: targetUrl, width: 1280, height: 800 }),
        });
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
        const data = await resp.json();
        const imageUrl = data.screenshotUrl || data.url || data.image;

        if (onProgress) {
          onProgress({ type: "get_page_screenshot", url: targetUrl, imageUrl });
        }

        return `Screenshot captured: ${imageUrl || 'available in response'}`;
      } catch (e) {
        return `Error: Screenshot capture failed. ${e.message || e}`;
      }
    }
